...
export default function POVIntake({ onCaseCreated, onMatches } : { onCaseCreated?: (caseId:string)=>void, onMatches?: (matches:any[])=>void }) {
  ...
  async function runIntake() {
    ...
    setLoading(true);
    const payload = {
      persona, owner: { name, role, email, org, originRegion }, objective, sector, budgetUSD: Number(budget || 0), timelineMonths: Number(timeline || 0),
      region: { name: targetRegion || 'Global - Unspecified', rawFeatures: parseFeatures(rawFeatures) }, openness, briefingFilePath: filePath
    };
    try {
      // Using absolute URL for safety in demo, typically would use relative or config
      const res = await fetch('http://localhost:5002/api/intake', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      const json = await res.json();
      setMatches(json.matches || []);
      if (onMatches) onMatches(json.matches || []);
    } catch (e) {
      console.error(e);
      alert('Intake failed - Ensure backend is running on port 5002');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function createCase() {
    ...
    try {
      const res = await fetch('http://localhost:5002/api/cases', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      const json = await res.json();
      if (json.case?.id) {
        alert('Case created: ' + json.case.id);
        if (onCaseCreated) onCaseCreated(json.case.id);
      }
    } catch (e) { console.error(e); alert('Create case failed'); }
  }
